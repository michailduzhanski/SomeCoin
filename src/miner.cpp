...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ArnakMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL))
        return error("ArnakMiner: ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ArnakMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("arnak-miner");
...
LogPrintf("BitcoinMiner: test 1\n");
            if (!pblocktemplate.get())
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in ArnakMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in ArnakMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintf("Running ArnakMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
LogPrintf("BitcoinMiner: tromp test 1\n");
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("ArnakMiner terminated\n");
        throw;
    }
