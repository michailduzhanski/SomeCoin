...
TEST(Mempool, PriorityStatsDoNotCrash) {
    ...
    // https://arnak.org/blog/security-announcement-2017-04-12.html
...
TEST(Mempool, SproutNegativeVersionTxWhenOverwinterActive) {
    ...
    // A Sprout transaction with version -3 is created using Sprout code (as found in arnakd <= 1.0.14).
    ...
    // A Sprout transaction with version -3 created using Overwinter code (as found in arnakd >= 1.0.15).
