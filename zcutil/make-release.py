...
def main(args=sys.argv[1:]):
    """
    Perform the final Arnak release process up to the git tag.
    """
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "arnak-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "arnak-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './arnak-make-release.log'
    ...
    logging.info('arnak make-release.py debug log: %r', logname)
