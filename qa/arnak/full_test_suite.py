#!/usr/bin/env python2
#
# Execute all of the automated tests related to Arnak.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/arnak/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/arnak/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if arnakd is an ELF binary, they all are
    with open(repofile('src/arnakd'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/arnakd')
    ret &= test_rpath_runpath('src/arnak-cli')
    ret &= test_rpath_runpath('src/arnak-gtest')
    ret &= test_rpath_runpath('src/arnak-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/arnakd')
    ret &= test_fortify_source('src/arnak-cli')
    ret &= test_fortify_source('src/arnak-gtest')
    ret &= test_fortify_source('src/arnak-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/arnak-gtest')],
