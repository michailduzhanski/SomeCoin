...
[ZIP208](https://github.com/arnak/zips/blob/master/zip-0208.rst) which shortens
...
One of the mechanisms that `arnakd` uses to detect whether it is in "initial
...
Starting from this release, `arnakd` additionally hard-codes the block hashes
...
surface of `arnakd`, we are removing `libsnark` from the codebase, and dropping
...
- `arnakd` no longer verifies old Sprout proofs, and will instead assume they
...
- Starting from this release, the circuit parameters from the original Sprout
  MPC are no longer required to start `arnakd`, and will not be downloaded by
...
It was vital to the success of Arnak, and the development of zero-knowledge
