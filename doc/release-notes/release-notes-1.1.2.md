...
In release 1.0.9 we implemented automatic deprecation of `arnakd` software
versions made by the Arnak Company. The configuration option
...
This release removes the `-disabledeprecation` option, so that `arnakd` software
versions made by the Arnak Company will always shut down in accordance with the
...
supported by the Arnak Company.
...
Jay Graber (8):
      Add Sapling key classes to wallet, with new librustarnak APIs
      Change librustarnak dependency hash to work for new Sapling classes
      Update librustarnak dependency, address comments
...
Simon Liu (12):
      ...
      Remove now redundant Rust call to librustarnak_xor.
