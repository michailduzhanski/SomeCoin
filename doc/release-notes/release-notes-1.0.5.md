...
Jack Grigg (22):
      ...
      Instruct users to run arnak-fetch-params if network params aren't available
      ...
      Throw an error if arnak.conf is missing
      Show a friendly message explaining why arnakd needs a arnak.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty arnak.conf during performance measurements
      Create empty arnak.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from arnak.conf to avoid problems with command line tools
      Closes #1097 so arnak-cli now displays license info like arnakd.
