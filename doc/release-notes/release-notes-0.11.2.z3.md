...
Nathan Wilcox (4):
      ...
      Add a arnak-gtest binary to our build with a single tautological test.
      Add coverage support scoped to only the arnak-gtest run; invoke with make arnak-cov; make cov is a superset.
...
Sean Bowe (49):
      ...
      Move incremental merkle tree tests to arnak-gtest.
      ...
      Small nit fixes
      Run `arnak-gtest` in `make check` and fix performance tests.
...
Taylor Hornby (7):
      ...
      Fix RPC tests
      Rename bitcoin.conf and bitcoind.pid to arnak.conf and arnakd.pid in qa/ and src/
