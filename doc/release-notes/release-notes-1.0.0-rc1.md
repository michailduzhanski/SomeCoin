...
Jay Graber (17):
      ...
      Refer to Arnak wiki in INSTALL
      Rm bitcoin logo
      Rm build-unix.md, to keep single copy of build instructions for Arnak on github wiki
      Rm Bitcoin-specific documentation
      Add note that document is not updated for Arnak to translation policy
      Rm doc for disabled REST interface
      Change alpha to beta testnet, add arnak hidden service
      Improve documentation on connecting to arnak hidden server
      Improve documentation on connecting to arnak hidden server
      Update tor.md
